...
export const getAggregatedIssuesWithVulnPaths = async (
  ...
  try {
    ...
    const [aggregatedIssuesResult, depGraphResult] = await Promise.all([
      aggregatedIssuesPromise,
      depGraphPromise,
    ]);
    const projectAggregatedIssues =
      aggregatedIssuesResult as AxiosResponse<AggregatedissuesPostResponseType>;
    const projectDepGraph =
      depGraphResult as AxiosResponse<DepgraphGetResponseType>;
